...
const Scraper = () => {
  ...
  const handleSearch = async (newSearch = true) => {
    ...
    try {
      ...
      const res = await axios.post(
        `${PORT}/api/search`, // Endpoint modified to remove '/api' if PORT already contains it.
  ...
  const renderRating = (rating) => {
    ...
    const fullStars = Math.max(0, Math.floor(Math.min(rating, 5))); // Ensure between 0-5
        ...
        {showResults && (
          <div className="fixed inset-0 pt-28 z-40 flex items-center justify-center bg-black/40 backdrop-blur-sm p-0">
            <div className=" w-screen h-screen bg-white shadow-2xl overflow-hidden animate-zoomIn">
              ...
              {/* Header */}
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between p-4 md:p-6 border-b bg-gradient-to-r from-indigo-50 to-white">
                <div>
